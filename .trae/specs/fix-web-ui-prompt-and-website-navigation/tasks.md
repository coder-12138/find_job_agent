...
- [ ] Task 3: 浏览器自动化与搜索工具增强
  ...
  - **Description**:
    ...
    - 两个新工具加入 `get_search_tools()` 返回列表
  - **Acceptance Criteria**: Agent 可调用工具查看页面按钮并点击；navigate_and_find_positions 能自动点击入口按钮
...
- [ ] Task 4: 系统提示更新与测试
  ...
  - **Description**:
    ...
    - 新增测试：对话续接测试、中断重启测试、入口按钮工具测试
    - 运行全部测试确保无回归
  - **Acceptance Criteria**: 全部测试通过，系统提示包含入口按钮探索与用户指导说明
